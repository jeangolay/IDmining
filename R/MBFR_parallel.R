...
#'
#' @usage MBFR_parallel(XY, scaleQ, m=2, C=NULL, ncores=4)
...
#' @param ncores Number of workers (by default: \code{ncores = 4})
...
#' @export
MBFR_parallel <- function(XY, scaleQ, m=2, C=NULL, ncores=4) {
  ...
  for (i in 1:C){
    ID_temp <- foreach(j=1:(E-i),.packages='data.table',.export="MINDID_SFS",
                       .combine='rbind')%dopar%{
                         xXsY <- cbind(X[,j,with=F],XsY)
                         xXs  <- xXsY[,-ncol(xXsY),with=F]

                         ID_XY <- round(MINDID_SFS(xXsY, scaleQ, m),2)
                         ID_X  <- round(MINDID_SFS(xXs, scaleQ, m),2)
                         Diss  <- round(ID_XY-ID_X,2)

                         matrix(c(ID_XY,ID_X,Diss),1,3)
                       }
